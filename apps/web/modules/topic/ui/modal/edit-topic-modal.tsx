...
export const EditTopicModal = () => {
  ...
  const { isOpen, onClose, topicId, name, subjectId, position, chapterId } =
    useEditTopic();
  const trpc = useTRPC();
  ...
  const subjectValue = watch("subjectId");

  // Populate form when data changes
  useEffect(() => {
    if (isOpen && name && subjectId && position && chapterId) {
      form.reset({
        name,
        subjectId,
        position,
  ...
  }, [isOpen, name, subjectId, position, form, chapterId]);

  const { mutate: updateTopic, isPending } = useMutation(
    trpc.admin.topic.updateOne.mutationOptions({
      onError: (err) => {
