...
import { Mcq, PublicExam } from "@workspace/db";
...
interface PublicExamWithRelation extends PublicExam {
  subjects: {
    subjectId: string;
    subject: {
      name: string;
...
export const PublicExamAssignQuestionView = ({
  exam,
  ...
  const { mutate: assignQuestionToExam, isPending } = useMutation(
    trpc.admin.mcq.assignQuestionToPublicExam.mutationOptions({
      onError: (err) => {
